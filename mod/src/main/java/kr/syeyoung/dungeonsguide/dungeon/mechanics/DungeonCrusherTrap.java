...
@Data
public class DungeonCrusherTrap implements DungeonMechanic {
    ...
    public void highlight(Color color, String name, DungeonRoom dungeonRoom, float partialTicks) {
        if (starting.getOffsetPointList().isEmpty()) return;
        OffsetPoint firstPoint = starting.getOffsetPointList().get(0);
        ...
        for (OffsetPoint offsetPoint : starting.getOffsetPointList()) {
            RenderUtils.highlightBlock(offsetPoint.getBlockPos(dungeonRoom), color, partialTicks);
        }
    }


    public DungeonCrusherTrap clone() throws CloneNotSupportedException {
        DungeonCrusherTrap dungeonSecret = new DungeonCrusherTrap();
    ...
    public OffsetPoint getRepresentingPoint(DungeonRoom dungeonRoom) {
        return starting.getOffsetPointList().size() == 0 ? null : starting.getOffsetPointList().get(0);
    }
}
