...
public class AbsLocationPopup extends AnnotatedImportOnlyWidget {
    ...
    public AbsLocationPopup(double x, double y, Widget child, boolean autoclose) {
        super(new ResourceIdentifier("dungeonsguide:gui/elements/locationed_popup.gui"));
        absX.setValue(x);
        absY.setValue(y);
    ...
    public void updatePos(double old, double neu) {
        ...
        if (Double.isInfinite(size.getWidth())) {
            size = new Size(0, size.getHeight());
        }
        if (Double.isInfinite(size.getHeight())) {
