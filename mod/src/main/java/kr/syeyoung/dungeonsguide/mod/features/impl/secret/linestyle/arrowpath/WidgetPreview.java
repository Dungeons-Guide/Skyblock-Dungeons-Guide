...
public class WidgetPreview extends Widget implements Renderer {
    ...
    private NeoRouteDisplayEngineRegistration.ArrowPathDisplayEngineSetting lineProperties;

    private static List<PathfindResult.PathfindNode> result = new ArrayList<>();
    static {
        result.add(new PathfindResult.PathfindNode(0.5, 1.0, 0.5, PathfindResult.PathfindNode.NodeType.WALK));
        result.add(new PathfindResult.PathfindNode(2.5, 1.0, 0.5, PathfindResult.PathfindNode.NodeType.WALK));
        result.add(new PathfindResult.PathfindNode(2.5, 1.0, 3.0, PathfindResult.PathfindNode.NodeType.WALK));
        result.add(new PathfindResult.PathfindNode(2.5, 1.5, 3.0, PathfindResult.PathfindNode.NodeType.WALK));
        result.add(new PathfindResult.PathfindNode(2.5, 1.5, 3.5, PathfindResult.PathfindNode.NodeType.WALK));
        result.add(new PathfindResult.PathfindNode(2.5, 2.0, 3.5, PathfindResult.PathfindNode.NodeType.WALK));
        result.add(new PathfindResult.PathfindNode(2.5, 2.0, 4, PathfindResult.PathfindNode.NodeType.WALK));
    }

    public WidgetPreview(NeoRouteDisplayEngineRegistration.ArrowPathDisplayEngineSetting lineProperties) {
...
//                if (x == 0 && z == 0) toDraw = Blocks.tnt.getBlockState();
        ...
        float scale = 0.2f;
        drawTextAtWorld("Destination", 2.5f, 2.5f + scale, 4.5f, 0xFF00FF00,lineProperties.getDestinationSize().floatValue()*2f, true, false, partialTicks);
        drawTextAtWorld("3.0m", 2.5f, 2.5f - scale, 4.5f, 0xFFFFFF00, lineProperties.getDestinationSize().floatValue()*2f, true, false, partialTicks);


        if (lineProperties.isBeacon()) {
            RenderUtils._renderBeaconBeam(2.5,2.5,4.5, lineProperties.getBeamColor(), partialTicks);
            GlStateManager.pushMatrix();
            RenderUtils._highlightBlock(new VectorI3D(2, 2, 4), lineProperties.getBeaconColor(), partialTicks, false);
            GlStateManager.popMatrix();
        }

        double speed = lineProperties.getAnimationSpeed();
        ...
        List<NeoRouteDisplayEngine.PathSegment> segments = NeoRouteDisplayEngine.transformPathfindResult(result, lineProperties.getLineWidth(), lineProperties.getLineSmoothness());
        NeoRouteDisplayEngine.drawLinesNormal(segments, 0, segments.size(), PathfindResult.PathfindNode.NodeType.WALK,
                lineProperties.getLineColor(),
                lineProperties.getArrowColor(),
                partialTicks,
                animate);
