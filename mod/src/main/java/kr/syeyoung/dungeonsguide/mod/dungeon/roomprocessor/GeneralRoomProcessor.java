    ...
    private ActionRoute getBestFit(float partialTicks) {
        ...
        for (ActionRoute value : path.values()) {
            ...
            if (value.getCurrentAction() instanceof ActionMove) {
                target = ((ActionMove) value.getCurrentAction()).getTarget().getPos(dungeonRoom);
                ...
                target = ((ActionMove)value.getActions().get(value.getCurrent()-1)).getTarget().getPos(dungeonRoom);
