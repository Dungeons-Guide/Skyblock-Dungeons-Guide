...
public class MapOverlayPlayer implements MapOverlay{
    ...
    public Vector3d getLocation(float partialTicks) {
        ...
        if (entityplayer != null && (!entityplayer.isInvisible() || entityplayer == Minecraft.getMinecraft().thePlayer)) {
            ...
            String iconName = context.getMapPlayerMarkerProcessor().getMapIconToPlayerMap().get(entry.getEffectiveName());
            if (iconName != null) {
                Vec4b vec = context.getScaffoldParser().getLatestMapData().mapDecorations.get(iconName);
                if (vec != null) {
                    BlockPos worldPt = context.getScaffoldParser().getDungeonMapLayout().mapPointToWorldPoint(new Point(vec.func_176112_b() / 2, vec.func_176113_c()/2));
                    return new Vector3d(worldPt.getX(), worldPt.getY(), vec.func_176111_d() * 360 / 16.0f);
                }
            }
        }
