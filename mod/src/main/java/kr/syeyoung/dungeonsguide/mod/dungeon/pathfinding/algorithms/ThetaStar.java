...
public class ThetaStar implements IPathfinder {
    ...
    private boolean lineOfSight(Node a, Node b) {
        ...
        for (int d = 0; d <= len; d += 1) {
            if (dungeonRoom.getBlock(Math.round(sx), (int) Math.ceil(sy), Math.round(sz)).isBlockedNonStonk()) return false;
            if (dungeonRoom.getBlock(Math.round(sx)+1, (int) Math.ceil(sy), Math.round(sz)+1).isBlockedNonStonk()) return false;
            if (dungeonRoom.getBlock(Math.round(sx)-1, (int) Math.ceil(sy), Math.round(sz)-1).isBlockedNonStonk()) return false;
            if (dungeonRoom.getBlock(Math.round(sx)+1, (int) Math.ceil(sy), Math.round(sz)-1).isBlockedNonStonk()) return false;
            if (dungeonRoom.getBlock(Math.round(sx)-1, (int) Math.ceil(sy), Math.round(sz)+1).isBlockedNonStonk()) return false;
            sx += dx; sy += dy; sz += dz;
