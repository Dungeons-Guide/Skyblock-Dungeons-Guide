...
public class RoomProcessorBlazeSolver extends GeneralRoomProcessor {
    ...
    public void drawWorld(float partialTicks) {
        ...
        if (!FeatureRegistry.getInstance().SOLVER_BLAZE.isEnabled()) return;
        if (next == null) return;
...
//            Gui.drawRect(-9999,-9999, 9999, 9999, 0xFFFFFFFF);
            ...
            RenderUtils.highlightBox(entity, AxisAlignedBB.fromBounds(-0.8,0, -0.8, 0.8, 2, 0.8), FeatureRegistry.getInstance().SOLVER_BLAZE.getBlazeColor(), partialTicks, false);
            if (entity == theoneafterit) {
                RenderUtils.highlightBox(entity, AxisAlignedBB.fromBounds(-0.8,0, -0.8, 0.8, 2, 0.8), FeatureRegistry.getInstance().SOLVER_BLAZE.getNextUpBlazeColor(), partialTicks, false);
            } else if (entity == nextBlaze)
                RenderUtils.highlightBox(entity, AxisAlignedBB.fromBounds(-0.8,0, -0.8, 0.8, 2, 0.8), FeatureRegistry.getInstance().SOLVER_BLAZE.getNextBlazeColor(), partialTicks, false);

            GlStateManager.color(1,1,1,1);


            if (FeatureRegistry.getInstance().SOLVER_BLAZE.<AColor>getParameter("blazeborder").getValue().getAlpha() > 0x10) {
                ...
                RenderUtils.highlightBox(entity, AxisAlignedBB.fromBounds(-1, 0, -1, 1, 2, 1), FeatureRegistry.getInstance().SOLVER_BLAZE.<AColor>getParameter("blazeborder").getValue(), partialTicks, false);
