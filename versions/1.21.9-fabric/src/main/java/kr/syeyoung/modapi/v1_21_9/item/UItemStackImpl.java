...
public class UItemStackImpl implements UItemStack {
    ...
    public Item getItem() {
        ...
        if (delegate.getItem() instanceof BlockItem) {
            Block terracottaBlock = ((BlockItem) delegate.getItem()).getBlock();
            if (terracottaBlock == Blocks.WHITE_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.ORANGE_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.MAGENTA_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.LIGHT_BLUE_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.YELLOW_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.LIME_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.PINK_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.GRAY_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.CYAN_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.PURPLE_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.BLUE_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.BROWN_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.GREEN_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.RED_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
            if (terracottaBlock == Blocks.BLACK_TERRACOTTA) return Item.STAINED_HARDENED_CLAY;
        }
        return Item.UNKNOWN;
    }
    ...
    public String getSkullTexture() {
        ...
        if (delegate.getItem() instanceof PlayerHeadItem item) {
            ...
            return component.getGameProfile().properties().get("textures").stream().findFirst().map(Property::value).orElse(null);
        }
        return null;
    }
    ...
    public CompoundBinaryTag serialize() {
        NbtElement compound = ItemStack.CODEC.encodeStart(MinecraftClient.getInstance().world.getRegistryManager().getOps(NbtOps.INSTANCE), delegate).getOrThrow();
