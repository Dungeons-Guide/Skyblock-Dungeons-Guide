...
public class EventListener {
    ...
    public void init() {
        ...
        HudElementRegistry.attachElementAfter(VanillaHudElements.MISC_OVERLAYS, Identifier.of("dungeonsguide", "hud"), (ctx, counter) -> {
    ...
    private void onScreenTick(Screen screen) {
        ...
        if (mouse.activeButton != null && (mouse.activeButton.button() != -1) && mouse.glfwTime > 0.0) {
            ...
            ModAPI.getAPI().getEventBus().fireEvent(new ScreenMouseEvent.MouseDragged(
                    f,g, (int) h, (int) i, mouse.activeButton.button()
            ));
        }
    }


    private boolean onScreenClick(Screen screen, Click context) {
        return !ModAPI.getAPI().getEventBus().fireEvent(new ScreenMouseEvent.MouseClicked(context.x(), context.y(), context.button(), false));
    }

    private boolean onScreenMouseRelease(Screen screen, Click context) {
        ModAPI.getAPI().getEventBus().fireEvent(new ScreenMouseEvent.MouseReleased(context.x(), context.y(), context.button()));
        return true;
    }
    ...
    private void onKeyRelease(Screen screen, KeyInput keyCode) {
        ModAPI.getAPI().getEventBus().fireEvent(new ScreenKeyboardEvent.KeyPressed(keyCode.key(), keyCode.scancode(), keyCode.modifiers()));
    }

    private void onKeyPress(Screen screen, KeyInput keyCode) {
        ModAPI.getAPI().getEventBus().fireEvent(new ScreenKeyboardEvent.KeyPressed(keyCode.key(), keyCode.scancode(), keyCode.modifiers()));
    }
    ...
    public void onGameMessage(Text message, boolean actionBar, Operation<Void> original) {
        ...
        if (event.isCanceled()) {
            ...
            ChatHudLine chatHudLine = new ChatHudLine(inGameHud.getTicks(), message, null, MessageIndicator.system());
