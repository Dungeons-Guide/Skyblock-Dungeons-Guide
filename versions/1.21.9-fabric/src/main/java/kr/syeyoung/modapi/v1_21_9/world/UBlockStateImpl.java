package kr.syeyoung.modapi.v1_21_9.world;
...
//        return delegat;
    ...
    public EnumDyeColor getColor() {
        ...
        Block block = this.block.getDelegate();
        if (block == Blocks.WHITE_TERRACOTTA) return EnumDyeColor.WHITE;
        if (block == Blocks.ORANGE_TERRACOTTA) return EnumDyeColor.ORANGE;
        if (block == Blocks.MAGENTA_TERRACOTTA) return EnumDyeColor.MAGENTA;
        if (block == Blocks.LIGHT_BLUE_TERRACOTTA) return EnumDyeColor.LIGHT_BLUE;
        if (block == Blocks.YELLOW_TERRACOTTA) return EnumDyeColor.YELLOW;
        if (block == Blocks.LIME_TERRACOTTA) return EnumDyeColor.LIME;
        if (block == Blocks.PINK_TERRACOTTA) return EnumDyeColor.PINK;
        if (block == Blocks.GRAY_TERRACOTTA) return EnumDyeColor.GRAY;
        if (block == Blocks.CYAN_TERRACOTTA) return EnumDyeColor.CYAN;
        if (block == Blocks.PURPLE_TERRACOTTA) return EnumDyeColor.PURPLE;
        if (block == Blocks.BLUE_TERRACOTTA) return EnumDyeColor.BLUE;
        if (block == Blocks.BROWN_TERRACOTTA) return EnumDyeColor.BROWN;
        if (block == Blocks.GREEN_TERRACOTTA) return EnumDyeColor.GREEN;
        if (block == Blocks.RED_TERRACOTTA) return EnumDyeColor.RED;
        if (block == Blocks.BLACK_TERRACOTTA) return EnumDyeColor.BLACK;
        if (delegate.getBlock() instanceof ConcretePowderBlock terracottaBlock) {
